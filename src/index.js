...
class MyMap {
  ...
  initCanvas() {
    ...
    this.canvas.style.height = `${this.canvasWidth * coordinateRatio}px`;
    this.canvas.width = `${this.canvasWidth * window.devicePixelRatio}`;
    this.canvas.height = `${this.canvasWidth * coordinateRatio * window.devicePixelRatio}`;
  ...
  drawBasicMap() {
    const gridPixel = this.canvasWidth / ((Math.abs(this.coordinate.bbox[0]) + Math.abs(this.coordinate.bbox[2])) / this.coordinate.grid);
    console.log(gridPixel);
    this.ctx.fillStyle = '#87bce1';
    this.globalRawData.forEach((row, x) => {
      row.forEach(y => {
        this.ctx.fillRect(x * gridPixel, y * gridPixel, gridPixel * 0.4, gridPixel * 0.4)
      })
    });
    console.log(this.globalRawData)
  }
}
